...
Sign-off checklist for Release Ops MVP. Each item maps to [specs.html §12 MVP Acceptance Criteria](specs.html#mvp). Use this document for release validation and operator handoff.

**Spec references:** [§12 MVP](specs.html#mvp) · [§9 Environment](specs.html#env) · [§10 Deployment](specs.html#deployment) · [§11 CI/CD](specs.html#ci)
...
- [ ] Backend calls provider-specific probe endpoints per [specs.html §6](specs.html#providers)
...
**How to verify:** Follow [getting-started.md](getting-started.md); confirm `docker compose config` shows a single service.

---

## AC #15 — GitHub Actions workflows

CI/CD layout per [specs.html §11](specs.html#ci). All entrypoints call reusable workflows; reusable workflows have no direct `on: push`.
