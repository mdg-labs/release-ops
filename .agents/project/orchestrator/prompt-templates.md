...
Examples:
  ...
  fix(db)[#42]: correct drizzle migration for releases table

Pre-commit:
  - Run SCOPED CI GATE (below) — failure → blocked, no commit
  - DB changes → npx drizzle-kit generate only (see DB MIGRATIONS)
...
- First shell action: npm install (fresh worktree has no node_modules)
